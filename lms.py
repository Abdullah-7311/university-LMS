...
Flask REST API with MySQL database support
Run: python3 lms_backend.py
"""
...
import hashlib
import datetime
import sqlite3  # Fallback if MySQL not available
...
# Change USE_MYSQL to True and fill credentials
# to use MySQL instead of SQLite
# ─────────────────────────────────────────────
USE_MYSQL = False  # Set True for MySQL
...
DB_FILE = "lms_database.db"  # SQLite fallback
...
def login():
    ...
    if role == 'student':
        ...
        if student:
            response_data['user']['student_id'] = student['id']
            response_data['user']['roll_number'] = student['roll_number']
            response_data['user']['department'] = student['department']
            response_data['user']['semester'] = student['semester']
...
def add_student():
    data = request.json
    required = ['username', 'password', 'full_name', 'roll_number']
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO students (user_id, roll_number, department, semester, batch_year, phone, address)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def update_student(student_id):
    ...
    try:
        cur.execute("""
            UPDATE students SET department=?, semester=?, batch_year=?, phone=?, address=?
            WHERE id=?
...
def add_course():
    ...
    try:
        cur.execute("""
            INSERT INTO courses (course_code, course_name, department, credits, semester, instructor, description)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def delete_course(course_id):
    conn, cur = get_connection()
    try:
        cur.execute("DELETE FROM marks WHERE course_id=?", (course_id,))
        cur.execute("DELETE FROM attendance WHERE course_id=?", (course_id,))
...
def add_marks():
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO marks (student_id, course_id, exam_type, marks_obtained, total_marks, grade, remarks, exam_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def mark_attendance():
    ...
    try:
        ...
        for rec in records:
            cur.execute("""
                INSERT OR REPLACE INTO attendance (student_id, course_id, date, status, remarks)
                VALUES (?, ?, ?, ?, ?)
...
def add_announcement():
    ...
    try:
        cur.execute("""
            INSERT INTO announcements (title, content, target_audience, department, posted_by)
            VALUES (?, ?, ?, ?, ?)
...
def get_timetable():
    ...
    query += " ORDER BY CASE t.day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 END, t.start_time"
    cur.execute(query, params)
    items = rows_to_list(cur.fetchall())
    conn.close()
...
def add_timetable():
    ...
    try:
        cur.execute("""
            INSERT INTO timetable (course_id, day_of_week, start_time, end_time, room, department, semester)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def admin_dashboard():
    ...
    cur.execute("""
        SELECT s.department, COUNT(*) as cnt FROM students s GROUP BY s.department
    """)
...
if __name__ == '__main__':
    ...
    print("🎒 Students       → username & password set by admin when adding a student")
    print("📌 Opening lms_frontend.html in your browser…")
    print("=" * 40)

    import threading, webbrowser, os, pathlib
    ...
    def open_browser():
        ...
        if html_path.exists():
            ...
            print("   Place lms_frontend.html in the same folder as lms_backend.py")
